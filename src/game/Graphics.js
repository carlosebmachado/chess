export default class Graphics {
  ...
  drawArrow(x1, y1, x2, y2, strokeStyle, lineWidth) {
    ...
    var headLen = (lineWidth || 3) * 3;
    var baseX = x2 - headLen * Math.cos(angle);
    var baseY = y2 - headLen * Math.sin(angle);
    ...
    this.ctx.lineTo(baseX - headLen * 0.4 * Math.cos(angle - Math.PI / 2), baseY - headLen * 0.4 * Math.sin(angle - Math.PI / 2));
    this.ctx.lineTo(baseX - headLen * 0.4 * Math.cos(angle + Math.PI / 2), baseY - headLen * 0.4 * Math.sin(angle + Math.PI / 2));
    this.ctx.closePath();
    this.ctx.fill();
  }
